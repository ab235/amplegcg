...
class MultiPromptAttack(object):
    ...
    def run_indiv_query(self, 
        ...
        for i in range(n_steps):
            ...
            if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                ...
                if i == n_steps-1:
                    self.log_indiv_query(i+1+anneal_from, n_steps+anneal_from, self.control_str, best_loss, runtime, model_tests, verbose=verbose, step_d = self.step_d)
                else:
                    self.log_indiv_query(i+1+anneal_from, n_steps+anneal_from, self.control_str, best_loss, runtime, model_tests, verbose=verbose)
    ...
    def run_multi_queries(self, 
        ...
        for i in range(n_steps):
            print("#"*10)
            print("In def run : n_steps",n_steps)
            print("#"*10)
            if stop_on_success:
                model_tests_jb, model_tests_mb, _ = self.test(self.workers, self.prompts)
                if all(all(tests for tests in model_test) for model_test in model_tests_jb):
            ...
            if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                ...
                model_tests = None
                self.log_multi_queries(i+1+anneal_from, n_steps+anneal_from, self.control_str, best_loss, runtime, model_tests, verbose=verbose)
    ...
    def log_indiv_query(self, step_num, n_steps, control, loss, runtime, model_tests = None, verbose=True, step_d = None):
    ...
    def log_multi_queries(self, step_num, n_steps, control, loss, runtime, model_tests, verbose=True):
        with open(self.logfile, 'r') as f:
            log = json.load(f)
...
class ProgressiveMultiPromptAttack(object):
    ...
    def run(self, 
        ...
        while step < n_steps:
            ...
            control, loss, inner_steps = attack.run_multi_queries(
